...
\begin{abstract}
We reproduce the convolutional neural-network (CNN) price-trend forecaster of
\citet{jiang2023reimagining} on a CRSP-style daily U.S. equity panel spanning
1992-01-01 to 2024-12-31. We re-implement the OHLC--volume image construction
(canvas sizes 15$\times$32, 60$\times$64 and 180$\times$96 for input horizons
$I\in\{5,20,60\}$ days), train horizon-matched CNN heads with the same conv/pool
recipe and a five-seed ensemble, and run a full out-of-sample long--short decile
backtest from 2001 onward with proportional transaction costs. Against
matched-horizon price-momentum, simple moving-average, and short-term reversal
baselines, the CNN long--short spread achieves an annualised Sharpe ratio of
$1.29$ at $I=5$, but only $0.16$ at $I=20$ and $-0.11$ at $I=60$. Out-of-sample
binary classification accuracy and AUC are $49.45\%/0.5103$, $52.68\%/0.5242$
and $53.70\%/0.5288$ respectively. A Fama--French five-factor plus momentum
spanning regression delivers a $+0.28\%$ annualised, statistically insignificant
alpha at $I=5$ but \emph{negative} alphas at $I=20$ ($-4.30\%$, $t=-2.23$) and
$I=60$ ($-6.65\%$, $t=-3.77$), partially diverging from the original paper.
Input-gradient saliency maps confirm that the CNN concentrates attention on
the most-recent price action and on volume contrast, consistent with a
data-driven momentum reading. All code and intermediate artefacts are released.
\end{abstract}
...
This paper reproduces the core pipeline from end to end and stress-tests it
against the same set of baselines on a longer post-publication sample. Our
contributions are:
\begin{enumerate}
  \item A self-contained re-implementation of the OHLC image renderer that
        matches the original PIL-level drawing rules pixel for pixel
        (including SMA overlay, volume sub-panel, and the top-to-bottom
        flip), as well as horizon-matched CNN backbones with the published
        kernel/stride/dilation/padding specification.
  \item A full 1992--2024 CRSP-style backtest with proportional transaction
        costs, matched-horizon price-momentum, simple moving-average cross,
        and short-term reversal baselines, plus a five-factor-plus-momentum
...
behaviour reported in the original paper. At $I=60$ the CNN ranking is
nearly horizontal in both our reproduction and the original figure,
...
with horizon, in line with the original paper's finding that longer
...
\begin{figure}[t]
  ...
  \caption{Input-gradient saliency for three high-confidence positive
    predictions of the $I=60$ ensemble. Left column: the original input
    image (inverted to a white background for readability). Right
...
Our reproduction reproduces the qualitative shape of
...
Three factors plausibly account for the gap relative to the original
paper. (i) Our CRSP-style file extends through 2024, well past the
post-publication window in which momentum-style signals have decayed
broadly. (ii) Our five-seed ensemble is markedly smaller than the
original ensembles, which increases variance in the predicted
probabilities. (iii) Differences in transaction-cost handling and
volume-bar normalisation across the published reference implementation
and our own may explain part of the residual gap at longer horizons.
