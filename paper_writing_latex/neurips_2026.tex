...
\citet{jiang2023reimagining} illustrate their CNNs using gradient-weighted class
activation mapping (Grad-CAM; \citealt{selvaraju2017grad}), producing
layer-wise heatmaps for an I20/R20 model (their Section~6.3 and Figure~13).
This replication instead reports pixel-level input-gradient saliency following
\citet{simonyan2014deep}---easier to implement in our codebase and focused on
...
is preserved (Table~\ref{tab:table2}, Figure~\ref{fig:fig7}), but the
...
Trading is tied to a \emph{fixed} calendar (Friday for $I{=}5$, month-end for
$I{=}20$, quarter-end for $I{=}60$), as in the label construction above. Spanning coefficients
in Table~\ref{tab:ff6} therefore describe how the CNN long--short return
covaries with risk factors \emph{conditional} on that timing; alternative
rebalance rules---e.g.\ shifting execution within the week or month to reduce
overlap with factor realisation windows---could change measured betas and
alphas, and systematic robustness along this dimension is left for future work.
The traditional-signal menu is also narrower than in Jiang et al.: we benchmark
the CNN against momentum, moving-average cross, and a simple five-day reversal
proxy rather than reproducing every STR/WSTR/TREND construction in the original
paper.
