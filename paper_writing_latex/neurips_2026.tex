...
returns in two panels: \subref{fig:fig8gross} is \emph{gross} of transaction costs;
\subref{fig:fig8net} applies the same long--short returns \emph{after} proportional
transaction costs (``net''), using identical volatility matching throughout. For
each panel, let $r_t^{s}$ be the periodic decile spread (gross in
\subref{fig:fig8gross}, net in \subref{fig:fig8net}), and let $r_t^{b}$ be the
benchmark holding-period return over the same interval. I compute
\[
  \tilde r_t^{s}
  =
  r_t^{s}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{Gross of transaction costs.}
    \label{fig:fig8gross}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figure5_vol_adjusted_cumlog_net.png}
    \caption{Net of proportional transaction costs (same $\sigma$-matching).}
    \label{fig:fig8net}
  \end{subfigure}
  \caption{Cumulative volatility-adjusted log returns of long--short portfolios.
    Panel~\subref{fig:fig8gross} uses decile spreads before transaction costs;
    Panel~\subref{fig:fig8net} uses the same construction after proportional costs.
    Each strategy is rescaled to the volatility of the benchmark measured over the
    same holding intervals.}
  \label{fig:fig8}
\end{figure}
...
volatility-adjusted return than the momentum and moving-average benchmarks in
\subref{fig:fig8gross}. Panel~\subref{fig:fig8net} shows that ranking is largely preserved net of costs, but cumulative slopes are lower and medium-horizon lines are closer to zero, in line with thinner net spreads. The
equal-weighted CNN curve is especially strong in the gross panel, while the value-weighted version
...
The longer-horizon CNN curves are flatter in both panels, which mirrors the weaker decile
...
\subref{fig:fig8gross} omits transaction costs for comparability with the
original paper's gross exposition, it should be read together with
\subref{fig:fig8net} and Table~\ref{tab:table2}: once turnover and transaction costs are applied, only
the weekly CNN spread retains a robust net Sharpe ratio in the tabulated
...
Table~\ref{tab:table2} confirms what Figure~\ref{fig:fig7} shows in cross-section together with Figure~\ref{fig:fig8} (risk-matched gross and net time series): only the $I=5$
horizon delivers a
...
Fifth, I omit Section~5 of the original paper, where they quantify \emph{time-scale transfer}---for instance, recycling \(I{=}5{/}R{=}5\) CNN estimates on densely sampled charts to forecasts from temporally aggregated long-horizon images while contrasting with lower-frequency architectures re-trained from scratch (summarized in Tables~8--9)---and \emph{international transfer}, which applies U.S.-trained CNN forecasts to offshore equity markets benchmarked country-by-country against locally estimated specifications (their Tables~10--11 and Figure~12). Every model reported here remains trained from scratch exclusively on CRSP-listed U.S. stocks at each horizon's native \(I{/}R\) pairing.
...
\item Figure~\ref{fig:fig8}
      (\path{paper_writing_latex/figures/figure5_vol_adjusted_cumlog.png} gross;
      \path{paper_writing_latex/figures/figure5_vol_adjusted_cumlog_net.png} net;
      ...
      \path{final_submit_version/outputs/pipeline_runs/figures/figure5_vol_adjusted_cumlog_net.png}%
      (duplicate slideshow copies under%
      \path{final_submit_version/outputs/ppt_images/}).\allowbreak{}
