...
spanning January 1, 1992, to December 31, 2024. Data are observed at the
...
Following \citet{jiang2023reimagining}, we encode each stock--date
...
\citet{jiang2023reimagining}, the volume chart is drawn along the bottom of
...
band. The $(W,H)$ pairs used in our implementation, identical to those reported
by \citet{jiang2023reimagining}, are:
\begin{center}
  \begin{tabular}{ccc}
    \toprule
    Horizon & $W$ & $H$ \\
...
Figure~\ref{fig:ohlc-sample} illustrates the three horizons for one security
on the same calendar date, so the reader can compare how a longer look-back
compresses the same price path into a wider and taller canvas.
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{ohlc_sample.png}
  \caption{OHLC-plus-volume images for a single security at horizons
    $I\in\{5,20,60\}$. The same calendar anchor (2015-12-18) is used so
...
\begin{figure}[H]
  ...
  \caption{Horizon-specific CNN backbones (cf.\ Figure~2 of
    \citet{jiang2023reimagining}). Each rectangle is one block:
...
\begin{figure}[t]
  ...
  \caption{Annualised decile returns by horizon for the CNN, matched-horizon
    ...
    \citet{jiang2023reimagining}; all series are computed from the same
...
\begin{table}[t]
  ...
  \caption{Out-of-sample binary classification metrics for the CNN
    ensemble. $N_{\mathrm{test}}$ is the number of rebalance-day
...
with \citet{jiang2023reimagining}, who argue that longer
